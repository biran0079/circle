...
class ContourFinder:
    ...
    def _update(self):
        if self.contours:
            for coll in self.contours.collections:
                coll.remove()
        self.contours = self.ax.contour(self.img, origin='image', levels=sorted(set(self.levels)))
    ...
    def render(self):
        ...
        for i in range(len(self.levels)):
            level = self.fig.add_axes([0.15, 0.04 * (i + 1), 0.60, 0.03], facecolor=axcolor)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--level_n', type=int, help='number of levels', default=1)
